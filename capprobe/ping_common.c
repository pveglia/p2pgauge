...
// please change the CPU_HZ to your CPU frequency
// the information can be found in /proc/cpuinfo
#define CPU_HZ 2519204000
...
#define CAP_MAX 10
double cap_rtt = 1000000000;
double cap_cap;			// Mbps
double cap_send_sec[CAP_MAX];
double cap_recv1_sec[CAP_MAX];
double cap_recv2_sec[CAP_MAX];
long cap_sn[CAP_MAX];
long cap_p = 0;			// pointer to the next entry
long cap_r = -1;		// pointer to the last received P-P
//------------------------------------------------
...
resend:
	gettimeofday(&tv, NULL);
printf("sending %ld  %ld  %ld %ld\n",sn,tv.tv_sec, tv.tv_usec,ntransmitted+1);
	sn += 2;

//-- cclljj
	cap_sn[cap_p] = ntransmitted + 1;
	cap_send_sec[cap_p] = (double)rdtsc()/CPU_HZ;
	cap_recv1_sec[cap_p] = -1;
	cap_recv2_sec[cap_p] = -1;
	cap_p++;
	cap_p %= CAP_MAX;
...
//			else
				printf(" time=%ld.%03ld ms", triptime/1000,
				       triptime%1000);
cap_r2 = cap_r;
for (kk=0;kk<CAP_MAX;kk++){
	cap_r2++;
	cap_r2 %= CAP_MAX;
	if (cap_sn[cap_r2]==seq){
		cap_recv1_sec[cap_r2] = (double)rdtsc()/CPU_HZ;
		break;
	} else if ((cap_sn[cap_r2]+1)==seq){
		cap_recv2_sec[cap_r2] = (double)rdtsc()/CPU_HZ;
		cap_r = cap_r2;
		if (cap_recv1_sec[cap_r2]>0){
			disp = cap_recv2_sec[cap_r2] - cap_recv1_sec[cap_r2];
			rtt = cap_recv1_sec[cap_r2] - cap_send_sec[cap_r2];
			rtt += cap_recv2_sec[cap_r2] - cap_send_sec[cap_r2];
			if (disp>0){
				cap = cc * 8 / (double)disp / 1000000;
				if (rtt<cap_rtt) {
					cap_rtt = rtt;
					cap_cap = cap;
				}
				printf("cap= %2.3lf Mbps C= %2.3lf Mbps",cap,cap_cap);
			} else printf ("disp=0");
		}
		break;
	}
